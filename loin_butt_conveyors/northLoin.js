class northLoin {
  ...
  update() {
    for (let i = 0; i < this.body.length; i++) {
      ...
      if (this.body[i].x >= 510 && this.body[i].x <= 830) {
        this.BendCoordinates[i].x = 0;
        this.BendCoordinates[i].y = 0;
      ...
      if (this.body[i].x <= 510 && this.body[i].x >= 470 && this.body[i].y < 120) {

        this.BendCoordinates[i].x = 510;
        this.BendCoordinates[i].y = 90;
        if (this.angle[i].x > -90) {
          this.angle[i].x -= conv3Speed * 180 / (2 * PI * (90 - 65));
        } else {
          this.body[i].x = this.body[i].x - (90 - 45);
          this.body[i].y = this.body[i].y + (90 - 45);
        }
      }
      if (this.body[i].x <= 470 && this.body[i].y <= 170 && this.body[i].x > 400) {
        this.BendCoordinates[i].x = 0;
        this.BendCoordinates[i].y = 0;
        this.body[i].x -= 0;
        this.body[i].y += conv5Speed;

      }
      if (this.body[i].y >= 170 && this.body[i].y <= 220 && this.body[i].x > 400) {
        this.BendCoordinates[i].x = 0;
        this.BendCoordinates[i].y = 0;
        if (this.angle[i].x >= -115) {
          this.angle[i].x -= 0.3;
        }
        this.body[i].x += conv5Speed / 2.3;
        this.body[i].y += conv5Speed / 1.11045046;
      }


      if (this.body[i].y >= 220 && this.body[i].y <= 230 && this.body[i].x > 400) {
        this.BendCoordinates[i].x = 0;
        this.BendCoordinates[i].y = 0;
        if (this.angle[i].x <= -90) {
          this.angle[i].x += 0.5;
        }
        if (this.body[i].x < 490) {
          this.body[i].x += conv5Speed / 2;
        }
        this.body[i].y += conv5Speed;
      }
      if (this.body[i].y >= 230 && this.body[i].y <= 360 && this.body[i].x > 400) {
        this.BendCoordinates[i].x = 0;
        this.BendCoordinates[i].y = 0;
        if (this.angle[i].x <= -90) {
          this.angle[i].x += 0.3;
      ...
      if (this.body[i].y >= 380 && this.body[i].y <= 420 && this.body[i].x >= 350 && this.body[i].x < 430) {
        ...
        this.body[i].x -= conv6Speed / 1.1;
        this.body[i].y += conv6Speed / 2.40039;
      }
  ...
  addProduct() {
    this.body[this.body.length] = createVector(width, 45);
